...
class EventMap extends Component {
  ...
  render() {
    ...
    const firstFutureEvent = _.chain(allEvents)
  ...
  _renderStaticUrlMarkerView(location) {
    return (
      <View>
        ...
        <Text style={[styles.calloutInfo, { color: '#aaa' }]}>{location.subtitle}</Text>
      </View>
    );
  }
  ...
  _renderStaticMarkerView(location) {
    return (
      <View>
        <View style={styles.calloutTitleWrap}>
          <Text style={[styles.calloutTitle, { color: '#333' }]}>{location.title}</Text>
        </View>
  ...
  _renderSwipeHelperOverlay() {
    return <View style={styles.swipeHelperArea} />;
  }
  ...
  _renderFilterSelectionButton(item) {
    return (
      <View key={item.id}>
        <TouchableOpacity
          onPress={this._changeShowFilter.bind(this, item.id)}
  ...
  _renderLocateMe() {
    return Platform.OS === 'ios' ? (
      <View style={styles.locateButton}>
        <TouchableOpacity
          onPress={this._toggleLocateMe.bind(this, null)}
          style={styles.locateButtonText}
        >
          <MDIcon
            size={20}
            style={{ color: this.props.locateMe ? '#1D7BF7' : '#888' }}
            name="navigation"
          />
        </TouchableOpacity>
      </View>
...
const select = store => {
  return {
    ...
    loading:
      store.event.get('listState') === LoadingStates.LOADING ||
