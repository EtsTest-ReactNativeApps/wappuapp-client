...
// # Selectors
export const getProfiles = state => state.user.getIn(['profile'], List()) || List();
export const getSelectedUser = state => state.user.get('selectedUser', Map()) || Map();

// Get user data
export const getSelectedUserData = createSelector(
  getSelectedUser,
  getProfiles,
...
export const getUserImage = state => state.user.getIn(['profile', 'profilePicture']);
...
export const openUserView = (user, avatar) => dispatch => {
  ...
  return dispatch({
    type: SET_USER_PROFILE,
    payload: Object.assign(user, { profilePicture: avatar, target: 'profile' }),
  });
};
