...
import { getUserTeamId } from './registration';

import config from '../config';
const cityKey = `${config.APP_STORAGE_KEY}:city`;
...
export const getCurrentCityName = createSelector(getCityId, getCityList, (cityId, cityList) => {
...
export const getCityIdByTeam = createSelector(getUserTeamId, getTeams, (teamId, teams) => {
...
export const getUsersCityName = createSelector(getCityIdByTeam, getCityList, (cityId, cities) => {
...
export const setCity = cityId => dispatch => {
  ...
  AsyncStorage.setItem(cityKey, JSON.stringify(cityId)).then(() =>
...
export const initializeUsersCity = () => (dispatch, getState) => {
  ...
  return AsyncStorage.getItem(cityKey)
    ...
    .catch(error => {
      console.log('error when setting city');
    });
};
